                      ...
                      <div className="px-4 pb-4 pointer-events-auto">
                        ...
                        {spark.isCollab && spark.status === 'pending' && !isOwnSpark ? (
                          ...
                          <>
                            ...
                            {/* Futuristic Dynamic Glass Actions Hub */}
                            <div className="relative flex justify-between items-center w-full h-[64px] bg-[#121212]/50 backdrop-blur-3xl rounded-full p-2 border border-white/10 shadow-[0_10px_40px_rgba(0,0,0,0.6)] overflow-hidden">
                              
                              {/* Left Text Box */}
                              <button
                                onClick={() => setActiveSheet("reply")}
                                className="relative h-full w-[45%] flex items-center justify-start px-4 gap-2 rounded-[24px] transition-all duration-300 bg-white/5 hover:bg-white/10 group"
                              >
                                <MessageCircle className="w-5 h-5 text-white/70 group-hover:text-white" />
                                <span className="text-white/70 font-semibold text-[13px] group-hover:text-white transition-colors truncate">
                                  Reply...
                                </span>
                              </button>
                              ...
                              {spark.isChallenge && (
                                <button
                                  ...
                                  className="absolute left-1/2 -translate-x-1/2 z-20 h-[110%] aspect-square bg-gradient-to-tr from-[#B026FF] to-[#00F0FF] rounded-full flex items-center justify-center p-0 shadow-[0_0_20px_rgba(176,38,255,0.4)] hover:scale-110 hover:rotate-[360deg] transition-all duration-700 hover:shadow-[0_0_30px_rgba(0,240,255,0.6)]"
                                >
                              ...
                              {/* Right Actions */}
                              <div className={`flex items-center justify-end gap-1.5 h-full ${spark.isChallenge ? 'flex-1 pl-6' : 'flex-1'} px-1`}>
                                <button
                                  onClick={() => setActiveSheet("share")}
                                  className="relative h-full flex-1 max-w-[60px] flex flex-col items-center justify-center rounded-full hover:bg-white/10 transition-colors group"
                                >
                                  <Share2 className="w-[20px] h-[20px] text-white/70 group-hover:text-[#00F0FF] group-hover:-translate-y-0.5 group-hover:translate-x-0.5 transition-all" />
                                </button>
                                
                                {isOwnSpark ? (
                                  <button
                                    onClick={handleSave}
                                    className={`relative h-full flex-1 max-w-[60px] flex items-center justify-center rounded-[24px] transition-all duration-300 ease-[cubic-bezier(0.23,1,0.32,1)] overflow-hidden group ${isSaved ? 'bg-emerald-500/20 shadow-[inset_0_0_15px_rgba(16,185,129,0.3)] border border-emerald-500/30' : 'bg-transparent hover:bg-white/10'}`}
                                  >
                                    {isSaved && <div className="absolute inset-0 bg-emerald-400/10 blur-xl" />}
                                    <Bookmark className={`relative z-10 transition-all duration-300 ${isSaved ? 'w-[20px] h-[20px] fill-emerald-400 text-emerald-400 drop-shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'w-[20px] h-[20px] text-white/70 group-hover:text-white'}`} />
                                  </button>
                                ) : (
                                  <button
                                    onClick={handleSave}
                                    className={`relative h-full flex-1 max-w-[60px] flex items-center justify-center rounded-[24px] transition-all duration-300 ease-[cubic-bezier(0.23,1,0.32,1)] overflow-hidden group ${isSaved ? 'bg-amber-500/20 shadow-[inset_0_0_15px_rgba(245,158,11,0.3)] border border-amber-500/30' : 'bg-transparent hover:bg-white/10'}`}
                                  >
                                    {isSaved && <div className="absolute inset-0 bg-amber-400/10 blur-xl" />}
                                    <Bookmark className={`relative z-10 transition-all duration-300 ${isSaved ? 'w-[20px] h-[20px] fill-amber-400 text-amber-400 drop-shadow-[0_0_8px_rgba(245,158,11,0.5)]' : 'w-[20px] h-[20px] text-white/70 group-hover:text-white'}`} />
                                  </button>
                                )}
                              </div>
                            </div>
