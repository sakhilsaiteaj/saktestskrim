...
export function ImmersivePostViewer({ initialIndex, type, urls, ids, user, onClose }: ImmersivePostViewerProps) {
  ...
  // We must track the current id
  const currentId = ids?.[currentIndex] || `unknown_${currentIndex}`;

  useEffect(() => {
    // Sync local storage on mount and index change
    if (currentId) {
      const savedList = JSON.parse(localStorage.getItem('skrimchat_saved_posts') || '[]');
      const savedSparks = JSON.parse(localStorage.getItem('skrimchat_saved_sparks') || '[]');
      setIsSaved(savedList.includes(currentId) || savedSparks.includes(currentId));
    }
    ...
    <div className="fixed inset-0 z-[110] bg-black overflow-hidden flex flex-col justify-center items-center">
      ...
      >
        <div className="max-w-2xl mx-auto space-y-4">
          <div className="cursor-pointer" onClick={() => setShowCommentsSheet(true)}>
          ...
          <div className="flex items-center gap-2 mb-4 cursor-pointer" onClick={() => setShowCommentsSheet(true)}>
          ...
          {/* Creative Action Hub */}
          <div className="relative w-full h-[64px] mb-6 flex items-center justify-between bg-[#121212]/60 backdrop-blur-3xl rounded-full p-2 border border-white/5 shadow-[0_8px_32px_rgba(0,0,0,0.5)] overflow-hidden">
             
             {/* Left - Dynamic Pulse Engine */}
             <button
               onClick={togglePulse}
               className={`relative h-full flex items-center justify-center rounded-full transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] overflow-hidden group ${pulsed ? 'w-[45%] bg-gradient-to-r from-[#B026FF] to-[#CC44FF] shadow-[inset_0_0_20px_rgba(255,255,255,0.3)]' : 'w-[20%] bg-white/5 hover:bg-white/10'}`}
             >
               {pulsed && (
                 <motion.div 
                   className="absolute inset-0 bg-[linear-gradient(90deg,transparent,rgba(255,255,255,0.4),transparent)] skew-x-[-20deg]"
                   initial={{ x: '-150%' }} animate={{ x: '150%' }} transition={{ duration: 1.5, repeat: Infinity, ease: 'linear' }}
                 />
               )}
               <div className="flex items-center justify-center gap-2 z-10 w-full px-2">
                 <Zap className={`shrink-0 transition-all duration-300 ${pulsed ? 'w-6 h-6 fill-white text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.8)]' : 'w-5 h-5 text-white/70 group-hover:text-white'}`} />
                 <AnimatePresence>
                   {pulsed && (
                     <motion.span 
                       ...
                       className="text-sm font-black text-white tracking-widest whitespace-nowrap drop-shadow-md"
                     >
                       {pulsesCount >= 1000 ? parseFloat((pulsesCount / 1000).toFixed(1)) + 'K' : pulsesCount}
                     </motion.span>
                   )}
                 </AnimatePresence>
               </div>
             ...
             {/* Middle - Social Modules */}
             <div className="flex items-center justify-center gap-1 h-full flex-1 px-1">
                 <button onClick={(e) => { setShowCommentsSheet(true); addRipple(e); }} className="relative h-full flex-1 flex flex-col items-center justify-center rounded-full md:hover:bg-white/10 transition-colors group">
                    <MessageCircle className="w-[22px] h-[22px] text-white/70 group-hover:text-[#00F0FF] group-hover:-translate-y-0.5 transition-all" />
                    <span className="text-[9px] font-bold text-white/50 group-hover:text-[#00F0FF] mt-0.5">{482 + comments.length - 3}</span>
                 </button>
                 <div className="w-[1px] h-1/3 bg-white/10 rounded-full" />
                 <button onClick={(e) => { setShowShareMenu(true); addRipple(e); }} className="relative h-full flex-1 flex items-center justify-center rounded-full md:hover:bg-white/10 transition-colors group">
                    <Share className="w-[20px] h-[20px] text-white/70 group-hover:text-amber-400 group-hover:-translate-y-[2px] group-hover:translate-x-[2px] transition-all" />
                 </button>
             </div>
             ...
             <button 
               onClick={(e) => { 
                addRipple(e);
                if (!isSaved) showToast("Secured in Vault! 🛡️");
                
                // Toggle saved state
                setIsSaved(!isSaved); 
                
                // Store/Remove from local storage
                if (currentId && !currentId.startsWith('unknown')) {
                    if (currentId.startsWith('spark_')) {
                        let sparkList = JSON.parse(localStorage.getItem('skrimchat_saved_sparks') || '[]');
                        if (!Array.isArray(sparkList)) sparkList = [];
                        if (sparkList.includes(currentId)) {
                             sparkList = sparkList.filter((id: string) => id !== currentId);
                             showToast("Removed from Vault");
                        } else {
                             sparkList.push(currentId);
                        }
                        localStorage.setItem('skrimchat_saved_sparks', JSON.stringify(sparkList));
                    } else {
                        let savedList = JSON.parse(localStorage.getItem('skrimchat_saved_posts') || '[]');
                        if (!Array.isArray(savedList)) savedList = [];
                        if (savedList.includes(currentId)) {
                             savedList = savedList.filter((id: string) => id !== currentId);
                             showToast("Removed from Vault");
                        } else {
                             savedList.push(currentId);
                        }
                        localStorage.setItem('skrimchat_saved_posts', JSON.stringify(savedList));
                    }
                    // Dispatch highlight event so IdentitiyScreen refreshes automatically if in same window
                    window.dispatchEvent(new Event('highlightSaved'));
                }
               }}
               className={`relative h-full flex items-center justify-center rounded-full transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] overflow-hidden group ${isSaved ? 'w-[30%] bg-emerald-500/20 shadow-[inset_0_0_15px_rgba(16,185,129,0.3)] border border-emerald-500/30' : 'w-[20%] bg-white/5 hover:bg-white/10'}`}
             >
               {isSaved && <div className="absolute inset-0 bg-emerald-400/10 blur-xl" />}
               <div className="flex items-center gap-2 z-10">
                 <Bookmark className={`shrink-0 transition-all duration-300 ${isSaved ? 'w-5 h-5 fill-emerald-400 text-emerald-400 drop-shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'w-5 h-5 text-white/70 group-hover:text-white'}`} />
               </div>
             </button>
          </div>
