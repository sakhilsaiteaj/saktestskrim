...
export function SparkCreator({ isOpen, onClose, onPost }: SparkCreatorProps) {
  ...
  const ALL_LANGUAGES = [
    "Hindi","Telugu","Tamil","Kannada","Malayalam","Marathi","Bengali","Gujarati","Punjabi","Odia","Urdu","Assamese","Bhojpuri","Hinglish",
    "Chinese","Japanese","Korean","Indonesian","Malay","Thai","Vietnamese","Filipino","Burmese","Khmer",
    "English","Spanish","French","German","Italian","Portuguese","Russian","Dutch","Polish","Ukrainian","Greek","Romanian","Swedish","Norwegian","Danish","Finnish","Czech","Hungarian",
    "Arabic","Persian","Turkish","Hebrew","Pashto","Kazakh",
    "Swahili","Amharic","Hausa","Yoruba","Zulu","Afrikaans","Somali",
    "Portuguese (Brazilian)","French (Canadian)","Haitian Creole",
    "Maori","Hawaiian","Samoan"
  ];

  const getCreatorLanguage = () => {
    try {
      const finalLangs = JSON.parse(localStorage.getItem("skrimchat_final_langs") || "null");
      return finalLangs?.languages?.[0] || "English";
    } catch {
      return "English";
    }
  };

  const [contentLangs, setContentLangs] = useState<string[]>([getCreatorLanguage()]);
  ...
  const handlePost = () => {
    ...
    const postData: any = {
      ...
      caption: caption,
      language: contentLangs[0],
      additionalLanguages: contentLangs.slice(1),
    ...
    setIsCollab(false);
    setCollabPartner(null);
    setContentLangs([getCreatorLanguage()]);
    setMode('select');
  ...
  const renderLanguageSelector = () => (
    <div className="bg-[#111] rounded-2xl border border-white/10 p-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        <Globe className="w-4 h-4 text-[#00F0FF]" />
        <span className="text-xs font-bold text-white/70 uppercase tracking-widest">Content Language</span>
      </div>
      <div className="flex flex-wrap gap-2 mb-2">
        {contentLangs.map((lang) => (
          <div key={lang} className="bg-[#B026FF]/20 border border-[#B026FF] text-white px-2 py-1 rounded-full text-xs font-semibold flex items-center gap-1">
            {lang}
            {contentLangs.length > 1 && (
              <button onClick={() => setContentLangs(prev => prev.filter(l => l !== lang))} className="hover:bg-white/20 rounded-full p-0.5 ml-1">
                <X className="w-3 h-3" />
              </button>
            )}
          </div>
        ))}
        {contentLangs.length < 3 && (
          <button 
            onClick={() => { setShowLangSelector(true); setLangSearch(''); }}
            className="bg-white/5 border border-white/10 text-white/60 hover:text-white px-2 py-1 rounded-full text-xs font-semibold"
          >
            + Add Language
          </button>
        )}
      </div>
      <p className="text-[10px] text-white/40">Tap + to change or add more (max 3)</p>
    </div>
  );

  const renderLanguageSheet = () => {
    if (!showLangSelector) return null;
    const filtered = ALL_LANGUAGES.filter(l => l.toLowerCase().includes(langSearch.toLowerCase()));
    ...
    return (
      <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center p-0 sm:p-4">
        ...
        <motion.div 
          ...
          className="relative w-full max-w-md bg-[#141414] border border-white/10 rounded-t-3xl sm:rounded-3xl p-6 h-[85vh] sm:h-[80vh] flex flex-col shadow-2xl"
        >
          <div className="w-12 h-1.5 bg-white/20 rounded-full mx-auto mb-6 shrink-0 sm:hidden" />
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-bold text-white flex items-center gap-2"><Globe className="w-5 h-5 text-[#B026FF]" /> Select Language</h2>
            <button onClick={() => setShowLangSelector(false)} className="text-white/50 hover:text-white"><X className="w-5 h-5"/></button>
          </div>
          ...
          <input 
            ...
            className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-[#B026FF] mb-4 shrink-0"
          />
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar pb-6 space-y-2">
            {langSearch ? (
              filtered.map(lang => (
                <button 
                  key={lang}
                  disabled={contentLangs.length >= 3 && !contentLangs.includes(lang)}
                  onClick={() => {
                    if (contentLangs.includes(lang)) {
                      if (contentLangs.length > 1) setContentLangs(prev => prev.filter(l => l !== lang));
                    } else {
                      if (contentLangs.length < 3) {
                        setContentLangs([...contentLangs, lang]);
                        setShowLangSelector(false); // Close on selection for quick addition
                      }
                    }
                  }}
                  className={`w-full text-left px-4 py-3 rounded-xl font-semibold text-sm transition-colors flex justify-between items-center ${contentLangs.includes(lang) ? 'bg-[#B026FF]/20 text-[#B026FF] border border-[#B026FF]' : 'bg-white/5 text-white/80 hover:bg-white/10 border border-transparent'} disabled:opacity-30 disabled:cursor-not-allowed`}
                >
                  {lang}
                  {contentLangs.includes(lang) && <span className="text-xs">Selected</span>}
                </button>
              ))
            ) : (
              // Grouped by popular first
              <>
                <p className="text-xs font-bold text-white/40 uppercase tracking-widest pl-1 mb-2">Selected</p>
                {contentLangs.map(lang => (
                  <button 
                    key={lang}
                    onClick={() => {
                    ...
                    className="w-full text-left px-4 py-3 rounded-xl font-semibold text-sm transition-colors flex justify-between items-center bg-[#B026FF]/20 text-[#B026FF] border border-[#B026FF] mb-2"
                  >
                    {lang}
                    <X className="w-4 h-4"/>
                  </button>
                ...
                <p className="text-xs font-bold text-white/40 uppercase tracking-widest pl-1 mt-6 mb-2">All Languages</p>
                {ALL_LANGUAGES.filter(l => !contentLangs.includes(l)).map((lang) => (
                   <button 
                    key={lang}
                    disabled={contentLangs.length >= 3}
                    onClick={() => {
                      if (contentLangs.length < 3) {
                        setContentLangs([...contentLangs, lang]);
                    ...
                    className="w-full text-left px-4 py-3 rounded-xl font-semibold text-sm transition-colors flex justify-between items-center bg-white/5 text-white/80 hover:bg-white/10 border border-transparent disabled:opacity-30 disabled:cursor-not-allowed mb-2"
                  >
                    {lang}
                  </button>
                ))}
